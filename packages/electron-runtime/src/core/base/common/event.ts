...
export namespace Event {
  ...
  export function once<T>(event: Event<T>): Event<T> {
    return (listener, thisArgs = null, disposables?) => {
      ...
      let didFire = false;
      const result: IDisposable = event(
        e => {
          if (didFire) {
            return;
          } else if (result) {
...
export class PauseableEmitter<T> extends Emitter<T> {
  ...
  private readonly _eventQueue: LinkedList<T> = new LinkedList<T>();
...
export class AsyncEmitter<T extends IWaitUntil> extends Emitter<T> {
  private _asyncDeliveryQueue?: Array<[Listener<T>, T, Array<Promise<any>>]>;

  async fireAsync(
    eventFn: (thenables: Array<Promise<any>>, listener: Function) => T,
  ): Promise<void> {
    if (!this._listeners) {
      return;
    }
    ...
    for (
      ...
      const thenables: Array<Promise<void>> = [];
      this._asyncDeliveryQueue.push([
        e.value,
        eventFn(
          thenables,
          typeof e.value === 'function' ? e.value : e.value[0],
        ),
        thenables,
      ]);
    }
...
export class EventBufferer {
  private readonly buffers: Function[][] = [];
...
export class Relay<T> implements IDisposable {
  readonly emitter: Emitter<T> = new Emitter<T>({
    onFirstListenerDidAdd: () => {
      this.listening = true;
