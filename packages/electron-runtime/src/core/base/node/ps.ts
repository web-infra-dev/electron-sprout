...
import { ProcessItem } from '../common/processes';
  ...
  return new Promise((resolve, reject) => {
    ...
    if (process.platform === 'win32') {
      ...
      try {
        ...
        mainLog.info('windows-process-tree does not exist in mac', error);
      }
    } else {
      // OS X & Linux
      function calculateLinuxCpuUsage() {
        ...
        let cmd = JSON.stringify(
          getPathFromAmdModule(require, './cpuUsage.sh'),
        );
        cmd += ` ${pids.join(' ')}`;
        ...
        if (err || stderr) {
          if (process.platform !== 'linux') {
            ...
            const cmd = JSON.stringify(
              getPathFromAmdModule(require, './node/ps.sh'),
            );
            exec(cmd, {}, (_err, _stdout, _stderr) => {
              if (_err || stderr) {
                reject(_err || new Error(_stderr.toString()));
              } else {
                parsePsOutput(_stdout, addToTree);
          ...
          exec(
            ...
            (_err, _stdout, _stderr) => {
              if (_err || stderr) {
                reject(err || new Error(_stderr.toString()));
              } else {
                parsePsOutput(_stdout, addToTree);
