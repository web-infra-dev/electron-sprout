...
import { isRootOrDriveLetter } from '../common/extpath';
...
export const MAX_FILE_SIZE =
  process.arch === 'ia32' ? WIN32_MAX_FILE_SIZE : GENERAL_MAX_FILE_SIZE;
export const MAX_HEAP_SIZE =
  process.arch === 'ia32' ? WIN32_MAX_HEAP_SIZE : GENERAL_MAX_HEAP_SIZE;
...
export function renameIgnoreError(
  oldPath: string,
  newPath: string,
): Promise<void> {
...
export function readFile(
  ...
  return promisify(fs.readFile)(path, encoding as any);
}

export async function mkdirp(path: string, mode?: number): Promise<any> {
