...
class ProtocolReader extends Disposable {
  ...
  readonly _onMessage: Emitter<ProtocolMessage> = this._register(
  ...
  public acceptChunk(data: VSBuffer | null): void {
    ...
    while (this._incomingData.byteLength >= this._state.readLen) {
      ...
      if (this._state.readHead) {
        ...
        // buff is the body
        const { messageType } = this._state;
        const { id } = this._state;
        const { ack } = this._state;
...
export class Protocol extends Disposable implements IMessagePassingProtocol {
  ...
  private readonly _socketWriter: ProtocolWriter;

  private readonly _socketReader: ProtocolReader;
...
export class Client<TContext = string> extends IPCClient<TContext> {
  ...
  get onClose(): Event<void> {
    return this.protocol.onClose;
  }

  constructor(
    private readonly protocol: Protocol | PersistentProtocol,
    id: TContext,
  ) {
    super(protocol, id);
  }
...
export class PersistentProtocol implements IMessagePassingProtocol {
  ...
  private readonly _outgoingUnackMsg: Queue<ProtocolMessage>;
  ...
  constructor(socket: ISocket, initialChunk: VSBuffer | null = null) {
    ...
    this._socketDisposables.push(
      this._socket.onClose(() => this._onSocketClose.fire()),
    );
    if (initialChunk) {
  ...
  private _sendKeepAliveCheck(): void {
    ...
    const timeSinceLastOutgoingMsg =
      Date.now() - this._socketWriter.lastWriteTime;
  ...
  private _recvKeepAliveCheck(): void {
    ...
    const timeSinceLastIncomingMsg =
    ...
    if (msg.type === ProtocolMessageType.Regular) {
      if (msg.id > this._incomingMsgId) {
        if (msg.id !== this._incomingMsgId + 1) {
          console.error(
            `PROTOCOL CORRUPTION, LAST SAW MSG ${this._incomingMsgId} AND HAVE NOW RECEIVED MSG ${msg.id}`,
          );
        }
  ...
  private _recvAckCheck(): void {
    ...
    if (
      ...
      ProtocolConstants.AcknowledgeTimeoutTime
    ) {
      // Trash the socket
      this._onSocketTimeout.fire(undefined);
      return;
