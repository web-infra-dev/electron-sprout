...
export class Client implements IChannelClient, IDisposable {
  ...
  constructor(
    ...
    private readonly options: IIPCOptions,
  ) {
  ...
  private get client(): IPCClient {
    if (!this._client) {
      ...
      forkOpts.env = assign(deepClone(process.env), {
      ...
      this.child.on('error', err =>
        console.warn(
          `IPC "${this.options.serverName}" errored with ${err?.message || ''}`,
        ),
      );

      this.child.on('exit', (code: any, signal: any) => {
        process.removeListener('exit' as 'loaded', onExit);
