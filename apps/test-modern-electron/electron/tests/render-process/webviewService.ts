...
// import TestDriver, { sleep } from '@modern-js/runtime/electron-test';
...
describe('test webviewService', () => {
  // if can call getAppVersion, then proved that the registerServices function will be ok!
  it('test webviewService.callWebview webviewBridge[registerServices、addWebview]', async () => {
    testDriver?.call({
      funcName: 'openDevTools',
      winName: 'main',
      args: [webviewId1],
    });
    const result = await testDriver?.call({
      ...
      winName: 'main',
      args: [webviewId1, 'getAppVersion'],
    });
    expect(result).toBe('13.4.0');
  });
